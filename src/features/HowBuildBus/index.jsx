...
export const HowBuildBus = () => {
  ...
  return (
    <div className="relative flex flex-col max-w-[1170px] w-full space-y-[100px]">
      <img className="absolute top-0 right-0 max-w-[45%] w-full" src="/img/howBuildBus/circle.png" alt="" />
      <div className="flex flex-col space-y-10 max-w-[600px]">
      ...
      <div className="flex w-full justify-between">
        {info.map((item, itemIndex) => {
          return (
            <div className={`overflow-hidden max-w-[280px] h-[330px] w-full flex flex-col ${item?.styleWrap} rounded-[30px]`} key={itemIndex}>
              <img className="-mt-5" src={item?.icon} alt="" />
              <span className="-mt-10 text-xl px-10 pb-[60px] text-center">{item?.text}</span>
            </div>
          )
        })}
      </div>
